// Daily scraper using API's original ranking
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          // Use API's original ranking - DO NOT override it
          console.log(`   📊 Using API's original rankings: ${products.map(p => p.ranking).join(', ')}`);
...
async function fetchAllProducts() {
  const allProducts = [];
  const limit = 12; // 12 products per request
  let offset = 0;
  let page = 1;
  ...
  while (allProducts.length < 100 && page <= 10) { // Max 10 pages
    ...
    if (products.length > 0) {
      const rankings = products.map(p => p.ranking);
      console.log(`   📊 API Rankings: ${Math.min(...rankings)} - ${Math.max(...rankings)}`);
    }
  ...
  console.log(`📊 Ranking range: ${Math.min(...allProducts.map(p => p.ranking))} - ${Math.max(...allProducts.map(p => p.ranking))}`);
...
async function insertDailyData(products, collectionDate) {
  ...
  try {
    ...
    for (const product of products) {
      ...
      const values = [
        collectionDate,
        product.ranking || 0, // Use API's original ranking
        product.video_id || '',
        product.video_url || '',
...
async function main() {
  try {
    console.log('🚀 Starting Daily Trending Products Scraper - Using API Ranking...');
    console.log(`⏰ Execution time: ${new Date().toISOString()}`);
    
    // Create table
    await createTable();
    
    // Fetch all products with API's original ranking
    ...
    // Show top 10 products with API's original rankings
