// Simple daily scraper using existing API
...
// Fetch data from local API with pagination
async function fetchTrendingData() {
  const allProducts = [];
  const limit = 20; // Fetch 20 products per request
  let offset = 0;
  let hasMore = true;
  
  console.log('📡 Fetching trending data with pagination...');
  ...
  return allProducts.slice(0, 100); // Ensure we don't exceed 100
}
...
async function fetchPage(offset, limit) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log(`   📊 Got ${products.length} products from this page`);
          resolve(products);
        } catch (error) {
          console.error(`❌ Error parsing page ${Math.floor(offset/limit) + 1}:`, error.message);
    ...
    req.on('error', (error) => {
      console.error(`❌ Error fetching page ${Math.floor(offset/limit) + 1}:`, error.message);
      resolve([]); // Return empty array on error
    });
    
    req.setTimeout(10000, () => {
      console.error(`⏰ Timeout fetching page ${Math.floor(offset/limit) + 1}`);
      req.destroy();
      resolve([]);
    });
...
async function main() {
  try {
    ...
    console.log('📡 Fetching trending data from local API...');
    const products = await fetchTrendingData();
...
module.exports = { main, createTable, insertDailyData, fetchTrendingData };
