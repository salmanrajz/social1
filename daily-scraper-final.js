// Daily scraper with correct pagination and ranking (1-100)
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          // Fix ranking to be sequential (1-100) based on our pagination
...
async function fetchAllProducts() {
  const allProducts = [];
  const limit = 12; // 12 products per request (as per your curl examples)
  let offset = 0;
  let page = 1;
  
  console.log('📡 Fetching 100 products with correct pagination...');
  console.log('📊 Pagination pattern:');
  console.log('   Page 1: offset=0, limit=12 → ranks 1-12');
  console.log('   Page 2: offset=1, limit=12 → ranks 13-24');
  console.log('   Page 3: offset=2, limit=12 → ranks 25-36');
  ...
  while (allProducts.length < 100 && page <= 10) { // Max 10 pages
    ...
    offset += 1; // Increment offset by 1 (not by limit)
    page++;
  ...
  console.log(`📊 Final ranking range: 1 - ${allProducts.length}`);
...
async function main() {
  try {
    console.log('🚀 Starting Daily Trending Products Scraper (100 Products) - Final Version...');
