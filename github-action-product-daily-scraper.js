// GitHub Action script for daily trending products scraper
...
async function fetchPage(offset, limit, days = 1, region = 'us') {
...
async function fetchAllProducts(days = 1, region = 'us') {
  const allProducts = [];
  const limit = 12;
  let offset = 0;
  let page = 1;
...
async function main() {
  try {
    ...
    // Fetch all products (days=1 for daily)
    const products = await fetchAllProducts(1, 'us');
    ...
    console.log('🎯 Daily Product Scraping Complete!');
